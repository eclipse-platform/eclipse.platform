...
public class Convert {
	...
	public static final String WIN32_LONG_PATH_PREFIX = "\\\\?\\"; //$NON-NLS-1$
	public static final String WIN32_UNC_LONG_PATH_PREFIX = "\\\\?\\UNC"; //$NON-NLS-1$
	...
	public static char[] toPlatformChars(String target) {
		...
		if (target.startsWith("\\\\")) { //$NON-NLS-1$
			...
			int prefixLength = WIN32_UNC_LONG_PATH_PREFIX.length();
			char[] result = new char[prefixLength + nameLength - 1];
			WIN32_UNC_LONG_PATH_PREFIX.getChars(0, prefixLength, result, 0);
			target.getChars(1, nameLength, result, prefixLength);
			return result;
		}
		//convert simple path of form c:\path to unicode form \\?\c:\path
		int nameLength = target.length();
		int prefixLength = WIN32_LONG_PATH_PREFIX.length();
		char[] result = new char[prefixLength + nameLength];
		WIN32_LONG_PATH_PREFIX.getChars(0, prefixLength, result, 0);
