...
public class ResourceStats {
	...
	/**
	 * A single timed occurrence of a traced event, passed from a
	 * <code>start...</code> method to {@link ResourceStats#end(Run)}.
	 */
	...
	/*
	 * Whether the debug option of the event is set. Only this bundle's own options,
	 * since a debug options listener is not notified about another bundle's; the
	 * global perf flag is read separately through PerformanceStats.ENABLED.
	 */
	...
	/**
	 * Re-reads the tracing options, so that tracing can be switched at runtime.
	 */
	...
	/**
	 * Starts a run that is only timed, not recorded as a performance event.
	 */
