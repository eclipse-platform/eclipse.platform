...
public class MessageBundleRefactoring extends Refactoring {
	...
	public RefactoringStatus checkFinalConditions(IProgressMonitor monitor) throws CoreException, OperationCanceledException {
		...
		fAccessorTypeBinding = computeAccessorClassBinding(SubMonitor.convert(monitor, 1));
		...
		ICompilationUnit[] affectedUnits = RefactoringSearchEngine.findAffectedCompilationUnits(SearchPattern.createPattern(fAccessorClass, IJavaSearchConstants.REFERENCES), RefactoringScopeFactory.create(fAccessorClass), SubMonitor.convert(monitor, 5), result);
		...
		for (ICompilationUnit unit : affectedUnits) {
			if (unit.equals(fAccessorClass.getCompilationUnit()))
				continue;
			processCompilationUnit(result, unit, SubMonitor.convert(monitor, 1));
		}
		processPropertiesFile(result, SubMonitor.convert(monitor, 1));
		return result;
	}
	...
	private void processCompilationUnit(RefactoringStatus result, ICompilationUnit unit, IProgressMonitor monitor) throws CoreException {
		...
		processAST(result, root, rewriter, SubMonitor.convert(monitor, 1));
